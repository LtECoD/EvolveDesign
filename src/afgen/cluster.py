...
def get_chain_weight(
    ...
    if chain.mol_type == prot_id:
        ...
        weight = 0  # will not sample chain being not protein
...
def get_interface_weight(
    ...
    if n_prot == 0:     # will not sample interface having no protein 
        weight = 0
    else:
    ...
    def sample(self, records: List[Record], random: RandomState) -> Iterator[Sample]:  # noqa: C901, PLR0912
        ...
        for record in records:
            for chain_id, chain in enumerate(record.chains):
                if (not chain.valid) or (chain.mol_type != const.chain_type_ids["PROTEIN"]):
                    continue
            ...
            for int_id, interface in enumerate(record.interfaces):
                if (not interface.valid) or (
                    (record.chains[interface.chain_1].mol_type != const.chain_type_ids["PROTEIN"]) and \
                        (record.chains[interface.chain_2].mol_type != const.chain_type_ids["PROTEIN"])):
                    continue
